...
func (s *subscription) process() {
	log := logging.S().With("subtree", s.subtree, "start_mvcc", s.startMVCC)
	...
	for i := 0; ; i++ {
		...
		if len(keys) > 0 {
			...
			for i, k := range keys {
				...
				payload := decodePayload(res[i], typ)
				sendFn(payload)
			}
		}
	...
	pendingKeys := func() (pending []string) {
		for {
			select {
			case msg := <-s.inCh:
				switch msg.Payload {
				case "set":
					if key := extractKey(msg); key != "" {
