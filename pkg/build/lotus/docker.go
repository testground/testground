...
var (
	_ api.Builder = &DockerLotusBuilder{}
)

// DockerLotusBuilder builds the test plan as a go-based container.
type DockerLotusBuilder struct {
	proxyLk sync.Mutex
}
...
func (b *DockerLotusBuilder) Build(ctx context.Context, in *api.BuildInput, output io.Writer) (*api.BuildOutput, error) {
	cfg, ok := in.BuildConfig.(*DockerLotusBuilderConfig)
	if !ok {
		return nil, fmt.Errorf("expected configuration type DockerLotusBuilderConfig, was: %T", in.BuildConfig)
	}
...
func (*DockerLotusBuilder) ID() string {
	return "docker:lotus"
}

func (*DockerLotusBuilder) ConfigType() reflect.Type {
	return reflect.TypeOf(DockerLotusBuilderConfig{})
}
...
func setupGoProxy(ctx context.Context, log *zap.SugaredLogger, cli *client.Client, buildNetworkID string, cfg *DockerLotusBuilderConfig) (proxyURL string, warn error) {
