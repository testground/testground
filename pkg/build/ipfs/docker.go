...
var (
	_ api.Builder = &DockerIPFSBuilder{}
)

// DockerIPFSBuilder builds the test plan as a go-based container.
type DockerIPFSBuilder struct {
	proxyLk sync.Mutex
}

type DockerIPFSBuilderConfig struct {
	Enabled    bool
	...
	CopyIPFS   bool   `toml:"copy_ipfs" overridable:"no"`
	IPFSPath   string `toml:"ipfs_path" overridable:"yes"`
...
func (b *DockerIPFSBuilder) Build(ctx context.Context, in *api.BuildInput, output io.Writer) (*api.BuildOutput, error) {
	cfg, ok := in.BuildConfig.(*DockerIPFSBuilderConfig)
	if !ok {
		return nil, fmt.Errorf("expected configuration type DockerIPFSBuilderConfig, was: %T", in.BuildConfig)
	}
	...
	var (
		...
		ipfssrc      = filepath.Join(in.Directories.SourceDir(), cfg.IPFSPath)
		...
		ipfsdst      = filepath.Join(tmp, "ipfs")
	...
	if cfg.CopyIPFS {
	  log.Infow("copying ipfs src", "ipfs_path", cfg.IPFSPath)
		// Copy the ipfs source; go-getter will create the dir.
		if err := getter.Get(ipfsdst, ipfssrc, getter.WithContext(ctx)); err != nil {
			return nil, err
		}
		if err := materializeSymlink(ipfsdst); err != nil {
			return nil, err
		}
	}
...
func (*DockerIPFSBuilder) ID() string {
	return "docker:ipfs"
}

func (*DockerIPFSBuilder) ConfigType() reflect.Type {
	return reflect.TypeOf(DockerIPFSBuilderConfig{})
}
...
func setupGoProxy(ctx context.Context, log *zap.SugaredLogger, cli *client.Client, buildNetworkID string, cfg *DockerIPFSBuilderConfig) (proxyURL string, warn error) {
