...
func UsesDataNetwork(runenv *runtime.RunEnv) error {
	...
	case targetmode:
		...
		for _, iname := range []string{"eth0", "eth1"} {
			...
			for _, addr := range addrs {
				...
				client.MustPublish(ctx, netTopic, addr.String())
			}
		}
		client.MustPublish(ctx, netTopic, endOfNetworks)
		runenv.RecordMessage("published my addresses from all networks to sync service. ready to be tested.")
		client.MustSignalEntry(ctx, "target-ready")
	...
	case pingmode:
		...
		client.MustSubscribe(ctx, netTopic, nwCh)
