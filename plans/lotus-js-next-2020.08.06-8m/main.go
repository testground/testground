...
func run(runenv *runtime.RunEnv) error {
	...
	case seq == 1: // genesis node
		...
		if runenv.StringParam("ssh-tunnel") != "\"\"" {
			...
			for i := 1; i <= runenv.TestInstanceCount; i++ {
				...
				ipfsGatewayForwardArg := fmt.Sprintf(
					"RemoteForward %v %v:8080",
					32000+i-1,
					nodeIPv4.IP.String(),
				)
				tunnelArgs = append(tunnelArgs, "-o", ipfsGatewayForwardArg)
			}
	...
	case seq >= 2: // additional nodes
		...
		runenv.RecordMessage("Jim0")
		localWalletAddr, err := api.WalletDefaultAddress(ctx)
		if err != nil {
			return err
		}

		runenv.RecordMessage("Jim1")
		count := 0
		for {
		  runenv.RecordMessage("Jim2 check balance")
			balance, err := api.WalletBalance(ctx, address)
			if err != nil {
		    runenv.RecordMessage("WalletBalance error: %v", err)
			} else {
        runenv.RecordMessage("Jim2a check balance done")
        if balance.Sign() > 0 {
          break
        }
      }
...
func setupIPFS(runenv *runtime.RunEnv, cmdNodeInitial *exec.Cmd) error {
	...
	outfile, err = os.Create("/outputs/ipfs-configs.out")
	if err != nil {
		return err
	}
